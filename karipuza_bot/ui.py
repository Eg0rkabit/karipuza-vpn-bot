...
def main_text(first_name: str | None = None) -> str:
    ...
    return (
        ...
        f"Привет{greeting}. Здесь можно оформить доступ, подключить VPN "
        "и получить помощь без сложных настроек.\n\n"
        "Одна подписка работает на телефоне и компьютере. "
        "Когда появятся новые страны, они добавятся в неё автоматически."
    )
