  ...
  return `
    <section class="panel">
      ...
      <div class="actions">
        ...
        <button class="btn green" data-open-sub>Открыть</button>
    ...
    <main class="view">
      ...
      <section class="panel">
        ...
        <div class="steps">
          ...
          <div class="step"><div class="step-num">2</div><div>Скопируйте или откройте ссылку подписки из этого раздела.</div></div>
        ...
        <div class="actions">
          ...
          <button class="btn" data-link="https://github.com/Happ-proxy/happ-desktop/releases">Компьютер</button>
          <button class="btn ghost" data-refresh>Обновить</button>
        </div>
      </section>
      ${
...
function currentView() {
  if (!state.me) {
    return `<div class="loading"><div class="brand-mark"><span></span><span></span><span></span></div><p>Загрузка</p></div>`;
  }
  const views = {
    home: homeView,
    plans: plansView,
...
async function handleClick(event) {
  ...
  try {
    ...
    } else if (target.dataset.openSub !== undefined) {
      const url = state.me?.subscription?.subscriptionUrl;
      if (url) openLink(url);
