...
Поля для будущей онлайн-оплаты через ЮKassa:
...
### Как подключать ЮKassa дальше

Сейчас бот и Mini App уже умеют хранить настройки ЮKassa в `.env`, но реальные
автоматические платежи пока не включены. Для включения нужны:
...
После этого следующий кодовый шаг:

- при выборе тарифа создавать платеж в ЮKassa;
- отдавать пользователю ссылку `confirmation_url`;
- принимать webhook `payment.succeeded`;
- по успешному webhook автоматически выдавать или продлевать подписку.

Webhook для ЮKassa должен быть на HTTPS. У ЮKassa для HTTP-уведомлений подходят
порты `443` или `8443`, поэтому текущий домен Mini App на HTTPS подходит для
будущей интеграции.
